...
router.get("/", jwtMiddleware, asyncHandler(async (_req, res) => {
    res.json({connections: listRtaConnections()});
}));
...
router.post("/connect", jwtMiddleware, asyncHandler(async (req, res) => {
    ...
    const connection = await createRtaConnection(requireAuthContext(req));
    res.status(201).json({connection});
}));

router.get("/:id", jwtMiddleware, asyncHandler(async (req, res) => {
    res.json({connection: getRtaConnection(req.params.id).snapshot()});
}));
...
router.post("/:id/subscribe", jwtMiddleware, asyncHandler(async (req, res) => {
    ...
    if (error) throw badRequest(error.message);
    const subscription = await getRtaConnection(req.params.id).subscribe(value.resourceUri);
...
router.post("/:id/unsubscribe", jwtMiddleware, asyncHandler(async (req, res) => {
    ...
    const {value, error} = schema.validate(req.body);
    if (error) throw badRequest(error.message);
    const result = await getRtaConnection(req.params.id).unsubscribe(value.subscriptionId);
    res.json(result);
}));
...
router.get("/:id/events", jwtMiddleware, asyncHandler(async (req, res) => {
    ...
    if (error) throw badRequest(error.message);
    const connection = getRtaConnection(req.params.id);
    res.json({
...
router.delete("/:id", jwtMiddleware, asyncHandler(async (req, res) => {
    res.json(closeRtaConnection(req.params.id));
}));
