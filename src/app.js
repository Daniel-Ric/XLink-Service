...
function colorMethod(m) {
    switch (m) {
        case "GET": return chalk.cyan.bold(m);
        case "POST": return chalk.green.bold(m);
        case "PUT": return chalk.magenta.bold(m);
        case "PATCH": return chalk.yellow.bold(m);
        case "DELETE": return chalk.red.bold(m);
        default: return chalk.white.bold(m);
    }
}
...
app.use((req, res, next) => {
    ...
    res.on("finish", () => {
        ...
        if (env.LOG_PRETTY) {
            const line = [
                timestamp(),
                badge(res.statusCode),
                colorMethod(req.method || "GET"),
                chalk.white(url),
                colorStatus(res.statusCode),
                colorTime(ms),
                chalk.dim(`#${id.slice(0, 6)}`)
            ].join(" ");
            console.log(line);
        } else {
