...
export async function getProfileSettings(xuid, xboxliveToken, settings) {
    const url = `https://profile.xboxlive.com/users/xuid(${xuid})/profile/settings?settings=${encodeURIComponent(
        settings
    )}`;
        ...
        try {
            ...
            if (s === 401 || s === 403) {
                throw internal("Failed to get profile settings", {
                    status: s,
                    body: errV3.response?.data || errV3.message
                });
            }
            try {
                ...
                throw internal("Failed to get profile settings", {
                    status: errV2.response?.status,
                    body: errV2.response?.data || errV2.message
                });
            }
        }
    }, 30000);
...
export async function getAchievements(xuid, xboxliveToken, { titleId } = {}) {
    try {
        const url = titleId
            ? `https://achievements.xboxlive.com/users/xuid(${xuid})/achievements?titleId=${encodeURIComponent(
                titleId
            )}`
    ...
    return cached(["presence", xuid], async () => {
        try {
            const { data } = await http.get(
                ...
                { headers: { "x-xbl-contract-version": 3, Authorization: xboxliveToken } }
            );
            return data;
        } catch (err) {
...
export async function getPresenceBatch(
    xuids = [],
    xboxliveToken,
    { level = "all", onlineOnly = false, locale } = {}
) {
    const users = (xuids || [])
        .filter(Boolean)
        .map(x => String(x))
...
export async function getTitleHub(
    xuid,
    xboxliveToken,
    { fields = ["detail", "image", "scid", "achievement"], maxItems = 100, locale } = {}
) {
    ...
    const url = `https://titlehub.xboxlive.com/users/xuid(${xuid})/titles/titlehistory/decoration/${encodeURIComponent(
        fieldsStr
    )}?maxItems=${encodeURIComponent(maxItems)}`;
    const key = [
        "titlehub",
        xuid,
        fieldsStr,
        maxItems,
        locale || env.ACCEPT_LANGUAGE || "en-US"
    ];
    return cached(
        key,
        async () => {
            try {
                ...
                if (status === 404) {
                    return {
                        titles: [],
                        totalCount: 0,
                        note: "No titles found (TitleHub returned 404)"
                    };
                }
                throw internal(
                    "Failed to get titles (titlehub)",
...
export async function resolveXuidByGamertag(gamertag, xboxliveToken) {
    ...
    try {
        const url = `https://profile.xboxlive.com/users/gt(${encodeURIComponent(
            gamertag
        )})/profile/settings?settings=Gamertag`;
        const { data } = await http.get(url, {
            headers: { "x-xbl-contract-version": 2, Authorization: xboxliveToken }
        });
...
export async function getPeople(xboxliveToken, maxItems = 200, locale) {
    try {
        const url = `https://peoplehub.xboxlive.com/users/me/people?maxItems=${encodeURIComponent(
            maxItems
        )}`;
        const { data } = await http.get(url, {
            headers: {
...
export async function getGameClips(
    xuid,
    xboxliveToken,
    { titleId, max = 24, since, continuationToken } = {}
) {
        ...
        async () => {
            try {
                ...
                try {
                    ...
                    throw internal(
                        "Failed to lookup XUID by gamertag",
                        eV2.response?.data || eV2.message
                    );
                }
            }
        },
