...
export async function getAchievements(xuid, xboxliveToken, { titleId } = {}) {
    try {
        const url = titleId
            ? `https://achievements.xboxlive.com/users/xuid(${xuid})/achievements?titleId=${encodeURIComponent(titleId)}`
...
export async function getTitleHub(
    ...
    const url =
        `https://titlehub.xboxlive.com/users/xuid(${xuid})/titles/titlehistory/decoration/` +
        encodeURIComponent(fieldsStr) +
...
export async function getXuidByGamertag(gamertag, xboxliveToken) {
    ...
    const call = (ver) =>
        http.get(url, { headers: { "x-xbl-contract-version": ver, Authorization: xboxliveToken } });

    try {
        const { data } = await call(3);
        return data?.profileUsers?.[0]?.id || null; // XUID
    } catch (eV3) {
