...
export async function getMCToken(sessionTicket) {
    try {
        ...
        const res = await http.post(AUTH_BASE, payload, { headers: { Accept: "application/json", charset: "utf-8", "content-type": "application/json", "user-agent": "MCPE/UWP" } });
        if (res.data?.result?.authorizationHeader) {
            return res.data.result.authorizationHeader;
        }
        const isString = typeof res.data === "string";
        throw internal("Failed to get Minecraft token", { status: 500, json: !isString ? res.data : undefined, htmlSnippet: isString ? res.data.slice(0, 800) : undefined });
    } catch (err) {
